...
int main(int argc, char **argv)
{
	...
	cmdline.add("-val_method", "validation method: pick best w over validation set or average value with early stopping", &val_method, "early_stopping");
  ...
  cmdline.add("-sigma", "if kernel is rbf2 or rbf3 this is the sigma", &sigma, 1.0);
	...
	int consecutive_unimproved = 0;
	int num_training_examples = 0;

	// early stopping mode - num_epochs will be max value
	if ((val_instances_filelist != "") && (val_method == "early_stopping"))
		num_epochs = 50;
	...
	if (val_instances_filelist != "")
		LOG(INFO) << "Training with validation set using the method: " << val_method;
	...
	for (uint epoch = 0; epoch < num_epochs; epoch++) {
		...
		for (uint i=0; i <  training_dataset.size(); i++) {
			...
			if (max_loss_in_epoch < loss) 
				max_loss_in_epoch = loss;

			avg_loss_in_epoch += loss/training_dataset.size();			

			
			// Preaspiration addition - after each training example - check the validations error
			if ( (val_instances_filelist != "") && (val_method == "pick_best_w"))
			{
				LOG(INFO) << "Validation...";
				...
				// if its the best w over validation set (for now) - save it in a file
				...
				LOG(INFO) << " average normalized loss = " << (avg_loss_in_epoch / (i+1))
				<< " | This validation error = " << this_w_loss;
			}
		...
		// Preaspiration addition - after each epoch, check the validation error on the given 
		// validation set we check the validation loss using avrage values of w (over all
		// the examples and epochs)
		if ( (val_instances_filelist != "") && (val_method == "early_stopping"))
		{
			LOG(INFO) << "Validation...";
			...
			for (uint ii=0; ii < val_dataset.size(); ++ii) {
				...
				VotLocation yy_hat;
				val_dataset.read(xx, yy);
				// predict with mean W values (use_avg_w=true)
				classifier.predict(xx, yy_hat, pos_only, true);
				int onset_loss = abs(yy.voice - yy_hat.voice);
				int offset_loss = abs(yy.burst - yy_hat.burst);
				this_w_loss += onset_loss+offset_loss;
			}
			...
			LOG(INFO) << " average normalized loss = " << avg_loss_in_epoch
			<< " | This validation error = " << this_w_loss;

			// reached best classifier on validation set - save it to file
			if (this_w_loss < best_validation_loss)
			{
				best_validation_loss = this_w_loss;
				consecutive_unimproved = 0;
				classifier.save_avg_w(classifier_filename);
			}
			else
				consecutive_unimproved += 1;

			// if the validation loss have not decreased for 2 epochs - finish here
			if (consecutive_unimproved == 3)
			{
				LOG(INFO) << "The loss on the validation-set has not decreased for 3 consecutive epochs - finish training here";
				break;
			}
		}
