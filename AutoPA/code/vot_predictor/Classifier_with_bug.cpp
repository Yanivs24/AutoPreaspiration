...
int Classifier::phi_pos_size = 83;
...
double Classifier::update( SpeechUtterance& x, VotLocation y, VotLocation &y_hat, bool vot_loss)
{
	...
	// delta_phi /= 2.0;
	real_loss = current_loss; // for debug
...
infra::vector_view Classifier::phi_pos(SpeechUtterance& x, VotLocation& y)
{
	...
	********************************************************************************************************/
	...
	// v_i is 50
	// mean of diff_means(alpha_autocorrelation, 5) over the range
	...
	// v_i is 51
	// max of diff_means(alpha_autocorrelation, 5) over the range
	...
	*/
	...
	// mean over (tb, tv) - mean over (tv, tv+50)
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	double mean_3_post_vot = 0.0;
	max_i = _min(y.voice+30, int(x.size()));
	...
	for (int i = y.voice; i <= max_i; i++)
		mean_3_post_vot += x.scores(i,4);
	mean_3_post_vot /= double(max_i-y.voice+1);

	// run over last 2 values - vi[58], vi[59]
	v[v_i-2] = mean_3_vot-mean_3_post_vot;
	v[v_i-1] = mean_4_vot-mean_4_post_vot;
	...
	mean_zc_vot /= double(y.voice-y.burst+1);

	// calc mean over post vot: (tv, tv+50)
	double mean_zc_post_vot = 0.0;
	for (int i = y.voice; i <= max_i; i++)
	...
	for (int i = y.voice+1; i <= max_i; i++)
		v[v_i] += x.scores(i, 3);
	v[v_i] /= double(max_i-y.voice);
	v_i++;
	
	// v_i is 62
	// Mean over wiener_entropy in (tv,tv+50)
	for (int i = y.voice+1; i <= max_i; i++)
		v[v_i] += x.scores(i, 4);
	v[v_i] /= double(max_i-y.voice);
	v_i++;
	...
	// v_i is 65
	// Max over high_energy in (tv,tv+50)
	v[v_i] = double(MISPAR_KATAN_MEOD);
	for (int i = y.voice+1; i <= max_i; i++)
	...
	// v_i is 66
	// Max over wiener_entropy in (tv,tv+50)
	v[v_i] = double(MISPAR_KATAN_MEOD);
	for (int i = y.voice+1; i <= max_i; i++)
...
/************************************************************************
 ...
 Comments:     none.
 ***********************************************************************/
double Classifier::predict(SpeechUtterance& x, VotLocation &y_hat, bool pos_only, bool use_avg_w)
{
	...
	for (int onset = 0; onset < max_onset; onset++) {
		...
		LOG(DEBUG) << "onset= " << onset << " min_vot= " << min_vot << " max_vot= " << max_vot << " x.size= " << x.size();
		for (int offset = min_vot; offset <= max_vot; offset++) {
			VotLocation y_temp;
			y_temp.burst = onset;
			y_temp.voice = offset;

			// use the mean of all the models until now (each call to update is considered a new model)
			if (use_avg_w)
				current_D = get_current_w_avg()*phi_pos(x,y_temp);
			// use the model's current w
			else
				current_D = w_pos*phi_pos(x,y_temp);

			LOG(DEBUG) << "Ts = " << y_temp.burst << " Te = " << y_temp.voice;
			LOG(DEBUG) << "phi = " << phi_pos(x,y_temp);
			LOG(DEBUG) << "wx = " << current_D;
			if (current_D > D_pos) {
				LOG(DEBUG) << "updating new max to: " << y_temp.burst << ", " << y_temp.voice;
...
/************************************************************************
 ...
 Comments:     none.
 ***********************************************************************/
void Classifier::save_avg_w(std::string &filename)
{
	std::ofstream ifs;
...
infra::vector Classifier::get_current_w_avg(void)
{
	return w_pos_sum / num_of_w_updates; 
}
