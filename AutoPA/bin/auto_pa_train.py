...
#
# auto_pa_train.py: Train a classifier to automatically measure PA,
# using manually annotated PAs in a set of textgrids and
...
if __name__ == "__main__":
    # parse arguments
    parser = argparse.ArgumentParser(description='Train a classifier to automatically measure PA, using manually '
                                                 'annotated PAs in a set of textgrids and corresponding wav files.  '
    ...
    parser.add_argument('--pa_tier', default='pa', help='Name of the tier to extract PAs from (default: %(default)s)')
    parser.add_argument('--pa_mark', default='*', help='Only intervals on the pa_tier with this mark value (e.g. '
                                                        '"pa", "pos", "neg") are used for training, or "*" for any '
                                                        'string (this is the default)')
    parser.add_argument('--window_min', help='Left boundary of the window (in msec) relative to the PA interval\'s '
                                             'right boundary. Usually should be negative, that is, before the PA '
    ...
    parser.add_argument("--cv_method", default='early_stopping', help='cross validation method: pick best w over'
                                                            ' validation set or average value with early stopping'
                                                            'possible values: "pick_best_w" or "early_stopping" (default)')
    ...
    parser.add_argument('--C', default=50, type=int, help='"Trade-off between regularization and loss (running PA)')
    args = parser.parse_args()

    logging_defaults(args.logging_level)

    # intermediate files that will be used to represent the locations
    # of the PAs, their windows and the location of the corresponding
    ...
    # Option 1: if --cv_auto flag given, hold out 15% of the data for cross-validation, use the remaining 85% for training
    if args.cv_textgrid_list == '' and args.cv_auto:
        logging.debug("Using 15% of the data for cross-validation")
        num_instances_for_training = int(num_instances * 0.85)
    ...
    ## Option 2: Otherwise, if the user specified data to be used for cross-validation, use that.
        ...
        # Training
        cmd_vot_training = 'VotTrain -verbose %s -pos_only -val_method %s -vot_loss -epochs 5 -loss_eps 4 -min_vot_length 5 -val_instances_filelist %s -val_labels_filename %s' \
                           ' -C %s %s %s %s ' % (args.logging_level, args.cv_method, features_filename_test, labels_filename_test, args.C,
                            features_filename_training, labels_filename_training, args.model_filename)
        easy_call(cmd_vot_training)
    ...
    ## Option 3: Otherwise, use all data for training, and don't do any cross-validation.
        ...
        labels_filename_training = labels_filename_rs
        # Training
        cmd_vot_training = 'VotTrain -verbose %s -pos_only -vot_loss -epochs 3 -loss_eps 4 -min_vot_length 5 ' \
                           '-C %s %s %s %s' % (args.logging_level, args.C, features_filename_training,
    ...
    if len(problematic_files):
        logging.warning("Classifiers trained using all files except %s files, where something was wrong:" % str(len(problematic_files)))
        with open('problematic_files.txt', 'w') as f:
            f.write(str(problematic_files))
