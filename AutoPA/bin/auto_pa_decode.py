...
#
# auto_pa_decode.py: Use an existing classifier to measure PA for
...
from auto_pa_extract_features import *
...
if __name__ == "__main__":
    # parse arguments
    parser = argparse.ArgumentParser(description='Use an existing classifier to measure PA for stops in a set of '
    ...
    parser.add_argument('textgrid_filenames', default='', help="Text file listing corresponding manually labeled "
                                                               "TextGrid files containing stops PA is to be measured for")
    ...
    parser.add_argument('--window_mark', help='PA is only predicted for intervals on the window tier with this mark '
                                              ...
                                              'default)', default='*')
    parser.add_argument('--window_min', help='Left boundary of the window (in msec) relative to the PA '
    ...
    parser.add_argument('--csv_file', help='Write a CSV file with this name with one row per predicited PA, '
    ...
    if out_file:
        out_file.writerow(['wav_file', 'time', 'pa', 'confidence'])
    ...
    for wav_file, textgrid_file in zip(wav_files, textgrid_files):
        ...
        # intermediate files that will be used to represent the locations of the PAs, their windows and the features
        ...
        final_pa_filename = my_basename + ".pa"
        ...
        # call the old VOT project binaries
        ...
        # decoding (i.e., generate PA predictions)
        cmd_vot_decode = 'VotDecode -verbose %s -pos_only -max_onset 200 -min_vot_length %d -max_vot_length %d ' \
                         '-output_predictions %s %s %s %s' % (args.logging_level, args.min_pa_length,
                                                              args.max_pa_length, preds_filename, features_filename,
        ...
        pa_predictions = [line.strip() for line in open(preds_filename)]
        for feature_filename, pa_prediction in izip(feature_filelist, pa_predictions):
        ...
        # add "AutoPA" tier to textgrid_filename
        textgrid = TextGrid()
        textgrid.read(textgrid_file)
        auto_pa_tier = IntervalTier(name='AutoPA', xmin=textgrid.xmin(), xmax=textgrid.xmax())
        auto_pa_tier.append(Interval(textgrid.xmin(), xmin_preds[0], ''))
        ...
        for i in xrange(len(xmin_preds) - 1):
            ...
            auto_pa_tier.append(Interval(xmin_preds[i], xmax_preds[i], 'pred'))
            # print xmin_preds[i], xmax_preds[i], mark_preds[i]
            auto_pa_tier.append(Interval(xmax_preds[i], xmin_preds[i + 1], ''))
            # print xmax_preds[i], xmin_preds[i+1], ''
        ## instead of mark_preds[i] (confidence number), just put 'pred' in the interval
        auto_pa_tier.append(Interval(xmin_preds[-1], xmax_preds[-1], 'pred'))
        # print xmin_preds[-1], xmax_preds[-1], mark_preds[-1]
        auto_pa_tier.append(Interval(xmax_preds[-1], textgrid.xmax(), ''))
        # print xmax_preds[-1], textgrid.xmax(), ''

        ## check if target textgrid already has a tier named
        ## "AutoPA", modulo preceding or trailing spaces or case. If
        ## so, action taken depends on if --ignore_existing_tiers flag
        ## invoked
        existing_tiers = [n.strip() for n in textgrid.tierNames(case="lower") if n.strip() == 'autopa']
        if len(existing_tiers) > 0:
            logging.warning("File %s already contains a tier with the name \"AutoPA\"" % textgrid_file)
            if args.ignore_existing_tiers:
                logging.warning("Writing a new AutoPA tier (in addition to existing one(s))")
                textgrid.append(auto_pa_tier)
                textgrid.write(textgrid_file)
            else:
                logging.warning("New \"AutoPA\" tier is NOT being written to the file.")
        ...
        if out_file:
            for i in xrange(len(xmin_preds) - 1):
                pa, conf = xmax_preds[i] - xmin_preds[i], mark_preds[i]
                out_file.writerow([wav_file, str(xmin_preds[i]), str(pa), str(conf)])
            pa, conf = xmax_preds[-1] - xmin_preds[-1], mark_preds[-1]
            out_file.writerow([wav_file, str(xmin_preds[-1]), str(pa), str(conf)])
